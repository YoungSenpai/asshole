...
},{"mini-signals":12}],173:[function(require,module,exports){
'use strict';
...
},{"./Port":176}],174:[function(require,module,exports){
'use strict';
...
},{"uuid":169}],175:[function(require,module,exports){
'use strict';
...
},{"./ForegroundHub":173,"./Pipe":174}],176:[function(require,module,exports){
"use strict";
...
var Storage = (function () {
  ...
  _createClass(Storage, null, [{
    key: 'set',
    value: function set(key, value) {
      if (['number', 'string'].indexOf(typeof value) !== -1 && Storage.storage[key] && Storage.storage[key] === value) {
        return;
      }

      Storage.storage[key] = value;

      if (!Storage.subscribers[key]) return;
      var _iteratorNormalCompletion = true;
      var _didIteratorError = false;
      var _iteratorError = undefined;

      try {
        for (var _iterator = Storage.subscribers[key][Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          var i = _step.value;
    ...
    value: function subscribe(key, callback) {
      if (!Storage.subscribers[key]) {
        Storage.subscribers[key] = new Array();
      }

      return Storage.subscribers[key].push(callback) - 1;
    }
  }, {
    key: 'unsubscribe',
    value: function unsubscribe(key, id) {
      if (!Storage.subscribers[key]) {
        return;
      }

      Storage.subscribers[key].splice(id, 1);
    }
  }, {
    key: 'storage',
    value: {},
...
var ConnectionButton = (function (_Component) {
  ...
  function ConnectionButton() {
    ...
    this.state = {
      state: _sharedPanelStorage2['default'].get('state'),
      enabled: _sharedPanelStorage2['default'].get('enabled')
    };

    _sharedPanelStorage2['default'].subscribe('state', (function (newValue) {
      this.setState({ state: newValue });
    }).bind(this));
    _sharedPanelStorage2['default'].subscribe('enabled', (function (newValue) {
      this.setState({ enabled: newValue });
    }).bind(this));

    /*Promos.motivation.then((motivation) => {
      return motivation.onStateChanged.add(function(result){
        if(result && result.state && result.state == 'complete') {
  ...
  _createClass(ConnectionButton, [{
    ...
    value: function render() {
      ...
      if (enabled && enabled == 'enabled') {
        title = _WdgtI18n.i18n.get('connectionButtonTitle' + capitalize(this.state.state));
      } else {
        title = _WdgtI18n.i18n.get('connectionButtonTitleDisabledShareReqired');
      }

      return _react2['default'].createElement(
        'div',
        { className: 'connection-button connection-status-' + this.state.state + ' ' + (enabled && enabled == 'enabled' ? 'enabled' : ''), onClick: enabled ? this.handleConnectButtonClick.bind(this) : null },
        _react2['default'].createElement(
          'svg',
          { className: 'outer-layer', width: '120px', height: '120px' },
          _react2['default'].createElement('path', { className: 'outer-circle',
            fill: 'none',
            stroke: '#FF4600',
            strokeOpacity: '.5',
            strokeWidth: '1',
        ...
        _react2['default'].createElement(
          'svg',
          { className: 'inner-layer', width: '120px', height: '120px' },
          _react2['default'].createElement('path', { className: 'inner-circle',
            fill: 'none',
            stroke: '#FF4600',
            strokeOpacity: '1',
            strokeWidth: '1',
        ...
        _react2['default'].createElement(
          ...
          _react2['default'].createElement('circle', { className: 'background-circle', cx: '60', cy: '60', r: '55' })
        ),
        _react2['default'].createElement(
          'div',
          { className: 'title' },
...
},{"../ConnectionController/ConnectionController":181,"./ConnectionButton.less":180,"Wdgt/I18n":170,"react":167,"shared/panel/Storage":177}],180:[function(require,module,exports){
(function() { var head = document.getElementsByTagName('head')[0]; var style = document.createElement('style'); style.type = 'text/css';var css = ".connection-button{position:relative;height:120px;width:120px}.connection-button .title{position:absolute;top:50%;left:50%;transform:translate(-50%, -50%);font-size:12px;color:#fff;text-align:center}.connection-button .outer-layer,.connection-button .inner-layer,.connection-button .background-circle{position:absolute}.connection-button .background-circle{border-width:0;transition:fill .5s;stroke-width:1px;fill:#ff4600;fill-opacity:0;transition:all .4s ease-in-out}.connection-button:hover .background-circle{fill-opacity:.2}.connection-button:active .background-circle{fill-opacity:1}@keyframes dash_inner{from{stroke-dasharray:0,1000}to{stroke-dasharray:345,1000}}@keyframes dash_outer{from{stroke-dasharray:0,1000}to{stroke-dasharray:377,1000}}@keyframes rotation{from{transform:rotate(0deg)}to{transform:rotate(359deg)}}@keyframes pulsing{from{opacity:.9;transform:scale(1, 1)}to{transform:scale(1.03, 1.03);opacity:.5}}.connection-button.connection-status-disconnected .inner-circle,.connection-button.connection-status-disconnected .outer-circle{stroke:#ff533f;transition:stroke-opacity 2s}.connection-button.connection-status-disconnected .outer-layer{animation:pulsing .7s linear 0s infinite alternate}.connection-button.connection-status-disconnected .background-circle{fill:#ff533f}.connection-button.connection-status-connected .inner-circle,.connection-button.connection-status-connected .outer-circle{stroke:#50e366;transition:stroke-opacity 2s}.connection-button.connection-status-connected .outer-layer{animation:pulsing .7s linear 0s infinite alternate}.connection-button.connection-status-connected .background-circle{fill:#50e366}.connection-button.connection-status-connecting .inner-circle,.connection-button.connection-status-disconnecting .inner-circle,.connection-button.connection-status-connecting .outer-circle,.connection-button.connection-status-disconnecting .outer-circle{stroke:#007aff}.connection-button.connection-status-connecting .outer-circle,.connection-button.connection-status-disconnecting .outer-circle{opacity:.5;stroke-dasharray:0,1000;animation:dash_outer 2.7s ease-in-out alternate infinite}.connection-button.connection-status-connecting .inner-circle,.connection-button.connection-status-disconnecting .inner-circle{stroke-dasharray:0,1000;animation:dash_inner 2s ease-in-out alternate infinite}.connection-button.connection-status-connecting .outer-layer,.connection-button.connection-status-disconnecting .outer-layer{animation:rotation 1.3s infinite linear}.connection-button.connection-status-connecting .inner-layer,.connection-button.connection-status-disconnecting .inner-layer{animation:rotation 1s infinite linear}.connection-button.connection-status-connecting .background-circle,.connection-button.connection-status-disconnecting .background-circle{fill:#007aff}.connection-button:not(.enabled){color:#999}.connection-button:not(.enabled) .inner-circle,.connection-button:not(.enabled) .outer-circle{stroke:#999}.connection-button:not(.enabled) .background-circle{fill:#999}";if (style.styleSheet){ style.styleSheet.cssText = css; } else { style.appendChild(document.createTextNode(css)); } head.appendChild(style);}())
...
var ConnectionController = (function () {
  ...
  _createClass(ConnectionController, [{
    key: 'handleConnectButtonClick',
    value: function handleConnectButtonClick() {
      var currentState = _sharedPanelStorage2['default'].get('state');
    ...
    value: function connect(params) {
      ...
      var zone = params.zone ? params.zone : _sharedPanelStorage2['default'].get('zone') ? _sharedPanelStorage2['default'].get('zone') : 'global',
          domain = params.domain ? params.domain : _sharedPanelStorage2['default'].get('domain'),
          location = params.location ? params.location : _sharedPanelStorage2['default'].get('location') ? _sharedPanelStorage2['default'].get('location') : 'optimal',
          url = params.url ? params.url : _sharedPanelStorage2['default'].get('url'),
          noReload = params.noReload ? params.noReload : false;

      var whatToDo = this.whatToDo(location);

      if (whatToDo == 'connect') {
        return chrome.sdk.proxy.connect({ domain: domain, country: location, mode: zone, noReload: noReload });
      } else {
        _sharedPanelStorage2['default'].set('state', 'disconnected');
        _sharedPanelStorage2['default'].set('shareHasToBeShown', 'yes');
        _sharedPanelStorage2['default'].set('enabled', 'disabled');
      }
    }
  }, {
    key: 'disconnect',
    ...
    key: 'whatToDo',
    value: function whatToDo(location) {
      var country = location ? location : _sharedPanelStorage2['default'].get('location');
      var isShared = _sharedPanelStorage2['default'].get('promosInfo').shareUs.showed;

      if (!isShared && country == 'us') {
        return 'share';
      }
...
},{"shared/panel/Storage":177}],182:[function(require,module,exports){
'use strict';
...
var Location = (function (_Component) {
  ...
  function Location() {
    ...
    this.locations = _sharedPanelStorage2['default'].get('locations');
    this.connectionController = new _ConnectionControllerConnectionController2['default']();

    this.state = {
      location: _sharedPanelStorage2['default'].get('location'),
      state: _sharedPanelStorage2['default'].get('state')
    };
    ...
    _sharedPanelStorage2['default'].subscribe('location', (function (newValue) {
  ...
  _createClass(Location, [{
    ...
    value: function handleLocationClick() {
      ...
      var location = selectNode.options[selectNode.selectedIndex].value;
      if (location == this.state.location) {
        return;
      }

      _sharedPanelStorage2['default'].set('location', location);

      if (_sharedPanelStorage2['default'].get('enabled') == 'disabled' && location != 'us') {
        _sharedPanelStorage2['default'].set('enabled', 'enabled');
      }

      if (_sharedPanelStorage2['default'].get('state') == 'connected') {
        this.connectionController.reconect();
      } else {
    ...
    key: 'render',
    value: function render() {
      var titleForCountry = function titleForCountry(country) {
        if (!country || country == '' || country == 'optimal') {
          return _WdgtI18n.i18n.get('locationSelectAutomatically');
        }

        var countryInfo = _countryData2['default'].countries[country.toUpperCase()];
        if (countryInfo) {
      ...
      for (var k in this.locations) {
        var option = _react2['default'].createElement(
          'option',
          { value: this.locations[k][0], key: this.locations[k][0] },
          titleForCountry(this.locations[k][0])
        );
      ...
      return _react2['default'].createElement(
        ...
        _react2['default'].createElement(
          'div',
          { className: 'title ' + this.state.state },
          _WdgtI18n.i18n.get('location')
        ),
        _react2['default'].createElement(
          'span',
          { className: 'text' },
          _react2['default'].createElement(
            ...
            titleForCountry(this.state.location),
            _react2['default'].createElement('span', { className: 'arrow' })
          ),
          _react2['default'].createElement(
            'select',
            { ref: 'select', defaultValue: this.state.location, onChange: this.handleLocationClick.bind(this) },
            locations
          )
        )
      );
...
},{"../ConnectionController/ConnectionController":181,"./Location.less":183,"Wdgt/I18n":170,"country-data":7,"react":167,"shared/panel/Storage":177}],183:[function(require,module,exports){
(function() { var head = document.getElementsByTagName('head')[0]; var style = document.createElement('style'); style.type = 'text/css';var css = ".location{text-align:center;color:#fff}.location .text{position:relative;font-size:13px;left:10px}.location .text select{border:none;background:transparent;outline:none;color:#fff !important;left:0;top:0;right:30px;bottom:0;width:100%;height:100%;position:absolute;opacity:0;-webkit-appearance:none;-moz-appearance:none;appearance:none;background:#22272d}.location .text .country-name{font-weight:bold}.location .text .arrow{display:inline-block;width:10px;height:6px;background-image:url('/assets/drop-down-arrow.png');margin:0 5px}@media (-webkit-min-device-pixel-ratio: 1.5), (min--moz-device-pixel-ratio: 1.5), (-o-min-device-pixel-ratio: 3/2), (min-resolution: 1.5dppx){.location .text .arrow{background-image:url(\"/assets/drop-down-arrow@2x.png\");background-size:10px 6px}}.location .title{text-align:center;font-size:12px}.location .title.connected{color:#50e366}.location .title.connecting{color:#007aff}.location .title.disconnected{color:#ff533f}";if (style.styleSheet){ style.styleSheet.cssText = css; } else { style.appendChild(document.createTextNode(css)); } head.appendChild(style);}())
...
var _ZoneZone = require('../Zone/Zone');

var _ZoneZone2 = _interopRequireDefault(_ZoneZone);

var _LocationLocation = require('../Location/Location');
...
var _PromosPromos = require('../Promos/Promos');

var _PromosPromos2 = _interopRequireDefault(_PromosPromos);

var _config = require('config');

var _config2 = _interopRequireDefault(_config);

var _UnavaliableUnavaliable = require('../Unavaliable/Unavaliable');

var _UnavaliableUnavaliable2 = _interopRequireDefault(_UnavaliableUnavaliable);

var _SettingsSettings = require('../Settings/Settings');

var _SettingsSettings2 = _interopRequireDefault(_SettingsSettings);
    ...
    value: function render() {
      ...
      switch (this.state.dialog) {
        ...
        case 'main':
          content = _react2['default'].createElement(
            ...
            _react2['default'].createElement('div', { className: 'world-map' }),
            _react2['default'].createElement(
              'div',
              { className: 'upper-block' },
              _react2['default'].createElement(_ZoneZone2['default'], null),
              _react2['default'].createElement(_LocationLocation2['default'], null)
            ),
            _react2['default'].createElement(_ConnectionButtonConnectionButton2['default'], null),
            _react2['default'].createElement(_PromosPromos2['default'], null)
          );
          break;
        case 'settings':
          content = _react2['default'].createElement(_SettingsSettings2['default'], null);
          break;
        case 'unavailable':
          return _react2['default'].createElement(_UnavaliableUnavaliable2['default'], null);
          break;
      }
      ...
      return _react2['default'].createElement(
        ...
        _react2['default'].createElement(
          ...
          _react2['default'].createElement(_ToolbarToolbar2['default'], null),
          content,
          _react2['default'].createElement(
            'div',
            ...
            _react2['default'].createElement(
              ...
              { className: 'version' },
              'version ',
              _config2['default'].application.version
            )
          )
        )
      );
...
},{"../ConnectionButton/ConnectionButton":179,"../Location/Location":182,"../Promos/Promos":190,"../Settings/Settings":194,"../Toolbar/Toolbar":196,"../Unavaliable/Unavaliable":198,"../Zone/Zone":200,"./Panel.less":185,"config":178,"react":167}],185:[function(require,module,exports){
(function() { var head = document.getElementsByTagName('head')[0]; var style = document.createElement('style'); style.type = 'text/css';var css = "@font-face{font-family:'Gotha Pro';src:url('/assets/fonts/GothaProReg.otf')}html,body{margin:0;padding:0;-moz-user-select:none;-webkit-user-select:none;user-select:none;font-family:'Gotha Pro';font-size:12px;color:#fff;-webkit-font-smoothing:antialiased;overflow:hidden}body>*{float:left}.panel{min-width:240px;color:red}.panel .spinner{position:absolute;z-index:1;height:100%;width:100%;opacity:.4;background:white}a{outline:none}*{-webkit-user-select:none;cursor:default}a,a *{outline:none;cursor:pointer}.dialog{background-color:#23282e;padding:.1px}@keyframes map_pulsing{from{transform:scale(1, 1)}to{transform:scale(1.2, 1.2)}}.dialog-main{width:260px}.dialog-main .dialog-content{position:relative;height:370px}.dialog-main .dialog-content .upper-block{position:absolute;top:15%;left:50%;transform:translate(-50%, -50%);width:100%}.dialog-main .dialog-content .upper-block>*{margin-top:5px}.dialog-main .dialog-content .connection-button{top:45%;left:50%;transform:translate(-50%, -50%)}.dialog-main .dialog-content .world-map{position:absolute;top:0;bottom:0;left:0;right:0;opacity:.1;background-image:url('/assets/world-map.png');background-position:center center;background-size:contain;background-repeat:no-repeat;transition:transform 1s;pointer-events:none}.dialog-main .dialog-content .world-map{animation:map_pulsing 12s ease-in-out 0s infinite alternate}.dialog-main .dialog-content .promos{position:absolute;bottom:0;width:100%}.version-holder{text-align:right;padding-right:20px;padding-bottom:7px;opacity:.3}.version-holder .version{color:white;font-size:8px;line-height:10px}";if (style.styleSheet){ style.styleSheet.cssText = css; } else { style.appendChild(document.createTextNode(css)); } head.appendChild(style);}())
...
var MobilePromo = (function () {
  function MobilePromo() {
    _classCallCheck(this, MobilePromo);
  }

  _createClass(MobilePromo, [{
    key: 'handleGooglePlayStoreClick',
    value: function handleGooglePlayStoreClick(event) {
      chrome.sdk.reporting.event('Viral', 'Click Button', 'Google Play').then(function () {
        _WdgtTabs.tabs.openPage("https://play.google.com/store/apps/details?id=com.northghost.touchvpn&utm_medium=referral&utm_source=TouchChromeExtension&utm_campaign=InstallAndroidApp");
      });
    }
  }, {
    key: 'handleAppleStoreClick',
    value: function handleAppleStoreClick(event) {
      chrome.sdk.reporting.event('Viral', 'Click Button', 'iTunes').then(function () {
        _WdgtTabs.tabs.openPage("https://itunes.apple.com/us/app/touch-vpn-unlimited-free-vpn/id991744383?utm_medium=referral&utm_source=TouchChromeExtension&utm_campaign=InstallIOSApp");
      });
    }
  }, {
    key: 'handleWindowsStoreClick',
    value: function handleWindowsStoreClick(event) {
      chrome.sdk.reporting.event('Viral', 'Click Button', 'Windows Store').then(function () {
        _WdgtTabs.tabs.openPage("https://www.microsoft.com/store/apps/9nblggh5gb3f");
      });
    }
  }, {
    key: 'render',
    value: function render() {
      chrome.sdk.reporting.event('Viral', 'Click Button', 'Windows Store');
      ...
      return _react2['default'].createElement(
        ...
        _react2['default'].createElement(
          ...
          _WdgtI18n.i18n.get('mobilePromoTitle')
        ),
        _react2['default'].createElement(
          'div',
          { className: 'buttons' },
          _react2['default'].createElement('div', { className: 'android-logo', onClick: this.handleGooglePlayStoreClick.bind(this) }),
          _react2['default'].createElement('div', { className: 'ios-logo', onClick: this.handleAppleStoreClick.bind(this) }),
          _react2['default'].createElement('div', { className: 'windows-logo', onClick: this.handleWindowsStoreClick.bind(this) })
        )
      );
    }
  }]);
...
exports['default'] = MobilePromo;
module.exports = exports['default'];

},{"./MobilePromo.less":187,"Wdgt/I18n":170,"Wdgt/Tabs":172,"react":167}],187:[function(require,module,exports){
(function() { var head = document.getElementsByTagName('head')[0]; var style = document.createElement('style'); style.type = 'text/css';var css = ".mobile-promo{margin:20px 0;-webkit-animation:fadein 2s}@-webkit-keyframes fadein{from{opacity:0}to{opacity:1}}.mobile-promo .bottom-text{font-size:12px;color:#fff;text-align:center;margin:0 20px 20px 20px}.mobile-promo .buttons{vertical-align:bottom;text-align:center}.mobile-promo .buttons div{width:66px;height:62px;margin:0 5px;background-repeat:no-repeat;display:inline-block;cursor:pointer;background-size:66px 62px}.mobile-promo .buttons .android-logo{background-image:url('/assets/google-play.png')}@media (-webkit-min-device-pixel-ratio: 1.5), (min--moz-device-pixel-ratio: 1.5), (-o-min-device-pixel-ratio: 3/2), (min-resolution: 1.5dppx){.mobile-promo .buttons .android-logo{background-image:url(\"/assets/google-play@2x.png\");background-size:66px 62px}}.mobile-promo .buttons .ios-logo{background-image:url('/assets/app-store.png')}@media (-webkit-min-device-pixel-ratio: 1.5), (min--moz-device-pixel-ratio: 1.5), (-o-min-device-pixel-ratio: 3/2), (min-resolution: 1.5dppx){.mobile-promo .buttons .ios-logo{background-image:url(\"/assets/app-store@2x.png\");background-size:66px 62px}}.mobile-promo .buttons .windows-logo{background-image:url('/assets/windows.png')}@media (-webkit-min-device-pixel-ratio: 1.5), (min--moz-device-pixel-ratio: 1.5), (-o-min-device-pixel-ratio: 3/2), (min-resolution: 1.5dppx){.mobile-promo .buttons .windows-logo{background-image:url(\"/assets/windows@2x.png\");background-size:66px 62px}}";if (style.styleSheet){ style.styleSheet.cssText = css; } else { style.appendChild(document.createTextNode(css)); } head.appendChild(style);}())
...
var PromoShareUs = (function (_Component) {
  _inherits(PromoShareUs, _Component);

  function PromoShareUs() {
    _classCallCheck(this, PromoShareUs);

    _get(Object.getPrototypeOf(PromoShareUs.prototype), 'constructor', this).call(this);

    this.state = {
      networks: ['facebook', 'twitter', 'google-plus'],
      mode: 'buttons'
    };
  }

  _createClass(PromoShareUs, [{
    key: 'handleShareButtonClick',
    value: function handleShareButtonClick(network) {
      this.setState({ mode: 'redirect' });
      var promosInfo = _sharedPanelStorage2['default'].get('promosInfo');
      promosInfo.shareUs = {
        showed: true,
        next: _config2['default'].promo.shareUs.interval + Date.now()
      };

      _WdgtStorage2['default'].set('Promos.info', promosInfo).then(function () {
        _sharedPanelStorage2['default'].set('promosInfo', promosInfo);
        chrome.sdk.reporting.event('share-us', 'share', network).then(function () {
          _WdgtTabs.tabs.openPage(_config2['default'].promo.shareUs[network].url, { mode: 'popup', width: _config2['default'].promo.shareUs[network].width, height: _config2['default'].promo.shareUs[network].height });
          window.close();
        });
      });
    }
    ...
    value: function render() {
      ...
      this.state.mode = this.props.mode ? this.props.mode : this.state.mode;
      delete this.props.mode;

      var content;
      var motivated = this.props.motivated;

      if (this.state.mode == 'buttons') {
        chrome.sdk.reporting.event('share-us', 'share-us-was-shown');
        content = _react2['default'].createElement(
          'div',
          { className: 'promo-mode-buttons' },
          _react2['default'].createElement(
            'div',
            { className: 'promo-title' },
            _WdgtI18n.i18n.get(motivated ? 'promoShareUsTitleConnectionMotivated' : 'promoShareUsTitle')
          ),
          _react2['default'].createElement(
            'div',
            ...
            this.state.networks.map(function (network) {
              return _react2['default'].createElement(
                'div',
                { className: 'promo-button promo-button-' + network, onClick: _this.handleShareButtonClick.bind(_this, network) },
      ...
      if (this.state.mode == 'redirect') {
        content = _react2['default'].createElement(
          'div',
          { className: 'promo-mode-thanks' },
          _react2['default'].createElement('div', { dangerouslySetInnerHTML: { __html: _WdgtI18n.i18n.get('promoShareUsThanksAndRedirect') } })
        );
      }
      ...
      if (this.state.mode == 'thanks') {
        content = _react2['default'].createElement(
          ...
          _react2['default'].createElement(
            'div',
            { className: '' },
            _WdgtI18n.i18n.get('promoShareUsThanks')
          )
        );
      }
      ...
      return _react2['default'].createElement(
        'div',
        { className: "promo promo-share-us " + (motivated ? "promo-motivated" : "") },
        content
      );
    }
  }]);
...
exports['default'] = PromoShareUs;
module.exports = exports['default'];

},{"./PromoShareUs.less":189,"Wdgt/I18n":170,"Wdgt/Storage":171,"Wdgt/Tabs":172,"config":178,"react":167,"shared/panel/Storage":177}],189:[function(require,module,exports){
(function() { var head = document.getElementsByTagName('head')[0]; var style = document.createElement('style'); style.type = 'text/css';var css = "@font-face{font-family:'SocialNetworks';src:url('/assets/social-networks.woff') format('woff');font-weight:normal;font-style:normal}.promo-title{color:#fff}.promo-share-us{padding-bottom:15px}.promo-share-us.promo-motivated{height:90px;-webkit-animation:fadein 2s}@-webkit-keyframes fadein{from{opacity:0}to{opacity:1}}.promo-buttons{text-align:center;-webkit-animation:fadein 2s}@-webkit-keyframes fadein{from{opacity:0}to{opacity:1}}.promo-buttons .promo-button{display:inline-block;position:relative;margin:8px;height:40px;width:40px;border-radius:5px;text-shadow:rgba(0,0,0,0.3) 0 2px 0;font-family:\"SocialNetworks\";font-size:20px;color:white}.promo-buttons .promo-button,.promo-buttons .promo-button *{cursor:pointer}.promo-buttons .promo-button .text{display:none}.promo-buttons .promo-button .logo:after{position:absolute;top:50%;left:50%;transform:translate(-50%, -50%);height:20px;width:20px}.promo-buttons .promo-button:hover{box-shadow:inset 0 -1.5em 2em rgba(255,255,255,0.1),inset 0 0 3.5em rgba(0,0,0,0.33)}.promo-buttons .promo-button.promo-button-facebook{background-color:#3b5998}.promo-buttons .promo-button.promo-button-facebook .logo:after{content:\"\\e60e\"}.promo-buttons .promo-button.promo-button-twitter{background-color:#00a9f1}.promo-buttons .promo-button.promo-button-twitter .logo:after{content:\"\\e605\"}.promo-buttons .promo-button.promo-button-google-plus{background-color:#dc4e41}.promo-buttons .promo-button.promo-button-google-plus .logo:after{content:\"\\e60a\"}.promo-motivated .promo-later{display:none}.promo-title{position:relative;text-align:center;font-size:12px;margin:0 20px}.promo-later{position:relative;text-align:center;color:#999;margin-top:16px;cursor:pointer;font-size:12px}.promo-mode-thanks{margin:50px 30px;text-align:center;color:#fff}";if (style.styleSheet){ style.styleSheet.cssText = css; } else { style.appendChild(document.createTextNode(css)); } head.appendChild(style);}())
...
var _MobilePromoMobilePromo = require('./MobilePromo/MobilePromo');

var _MobilePromoMobilePromo2 = _interopRequireDefault(_MobilePromoMobilePromo);

var _RateUsPromoRateUsPromo = require('./RateUsPromo/RateUsPromo');

var _RateUsPromoRateUsPromo2 = _interopRequireDefault(_RateUsPromoRateUsPromo);

var _PromoShareUsPromoShareUs = require('./PromoShareUs/PromoShareUs');

var _PromoShareUsPromoShareUs2 = _interopRequireDefault(_PromoShareUsPromoShareUs);

var Promos = (function (_Component) {
  ...
  function Promos() {
    ...
    _get(Object.getPrototypeOf(Promos.prototype), 'constructor', this).call(this);
    ...
    _sharedPanelStorage2['default'].subscribe('state', function (newValue) {
      if (_this.unmount || _this.state.promo || _sharedPanelStorage2['default'].get('state') != 'connected') {
        return;
      }
    ...
    _sharedPanelStorage2['default'].subscribe('shareHasToBeShown', function (shareHasToBeShown) {
      if (shareHasToBeShown == 'yes' && _this.state.promo != 'shareUs') {
        _this.setState({ promo: 'shareUs' });
      }
    });
  }
  ...
  _createClass(Promos, [{
    ...
    value: function getPromoName() {
      ...
      if (!promosInfo.shareUs.showed || !promosInfo.shareUs.next || promosInfo.shareUs.next < Date.now()) {
        promosArray.push('shareUs');
      }
    ...
    value: function render() {
      var promo;
      var promoName = this.state.promo;

      if (promoName == 'mobile') {
        promo = _react2['default'].createElement(_MobilePromoMobilePromo2['default'], null);
      } else if (promoName == 'rateUs') {
        promo = _react2['default'].createElement(_RateUsPromoRateUsPromo2['default'], null);
      } else if (promoName == 'shareUs') {
        promo = _react2['default'].createElement(_PromoShareUsPromoShareUs2['default'], null);
      }
...
},{"./MobilePromo/MobilePromo":186,"./PromoShareUs/PromoShareUs":188,"./Promos.less":191,"./RateUsPromo/RateUsPromo":192,"Wdgt/I18n":170,"react":167,"shared/panel/Storage":177}],191:[function(require,module,exports){
(function() { var head = document.getElementsByTagName('head')[0]; var style = document.createElement('style'); style.type = 'text/css';var css = ".promos:empty{display:none}";if (style.styleSheet){ style.styleSheet.cssText = css; } else { style.appendChild(document.createTextNode(css)); } head.appendChild(style);}())
...
var RateUsPromo = (function (_Component) {
  _inherits(RateUsPromo, _Component);

  function RateUsPromo() {
    _classCallCheck(this, RateUsPromo);

    _get(Object.getPrototypeOf(RateUsPromo.prototype), 'constructor', this).call(this);
  ...
  _createClass(RateUsPromo, [{
    key: 'handleStarClick',
    value: function handleStarClick(rating) {
      var _this = this;

      chrome.sdk.reporting.event('rate-us', 'rate', rating + ' stars', rating).then(function () {
        var promosInfo = _sharedPanelStorage2['default'].get('promosInfo');
        promosInfo.rateUs = {
          showed: true,
    ...
    value: function render() {
      ...
      if (this.state.mode == 'stars') {
        chrome.sdk.reporting.event('rate-us', 'rate-us-was-shown');
        content = _react2['default'].createElement(
          'div',
          { className: 'promo-mode-stars' },
          _react2['default'].createElement(
            'div',
            { className: 'promo-title' },
            _WdgtI18n.i18n.get('promoRateUsTitle')
          ),
          _react2['default'].createElement(
            'div',
            { className: 'promo-stars' },
            [1, 2, 3, 4, 5].reverse().map(function (rating) {
              return _react2['default'].createElement('div', { className: 'promo-star', onClick: _this2.handleStarClick.bind(_this2, rating) });
            })
          )
        );
      }
      ...
      if (this.state.mode == 'thanks') {
        content = _react2['default'].createElement(
          ...
          _react2['default'].createElement(
            'div',
            { className: '' },
            _WdgtI18n.i18n.get('promoRateUsThanks')
          )
        );
      }
      ...
      if (this.state.mode == 'redirect') {
        content = _react2['default'].createElement(
          'div',
          { className: 'promo-mode-thanks' },
          _react2['default'].createElement('div', { dangerouslySetInnerHTML: { __html: _WdgtI18n.i18n.get('promoRateUsThanksAndRedirect') } })
        );
      }

      return _react2['default'].createElement(
        'div',
        { className: 'promo promo-rate-us' },
        content
      );
    }
  }]);
...
exports['default'] = RateUsPromo;
module.exports = exports['default'];

},{"./RateUsPromo.less":193,"Wdgt/I18n":170,"Wdgt/Storage":171,"Wdgt/Tabs":172,"config":178,"react":167,"shared/panel/Storage":177}],193:[function(require,module,exports){
(function() { var head = document.getElementsByTagName('head')[0]; var style = document.createElement('style'); style.type = 'text/css';var css = ".promo-rate-us{padding:20px 0;-webkit-animation:fadein 2s}@-webkit-keyframes fadein{from{opacity:0}to{opacity:1}}.promo-rate-us .promo-stars{margin:10px 20px;text-align:center;direction:rtl}.promo-rate-us .promo-title,.promo-rate-us .promo-mode-thanks,.promo-rate-us .promo-mode-store{margin:10px 20px;text-align:center;color:#fff;font-size:12px}.promo-rate-us .promo-star{display:inline-block;height:24px;width:24px;padding:2px;background-repeat:no-repeat;background-position:center center;background-image:url('/assets/star-unfilled.png')}@media (-webkit-min-device-pixel-ratio: 1.5), (min--moz-device-pixel-ratio: 1.5), (-o-min-device-pixel-ratio: 3/2), (min-resolution: 1.5dppx){.promo-rate-us .promo-star{background-image:url(\"/assets/star-unfilled@2x.png\");background-size:24px 24px}}.promo-rate-us .promo-star:hover,.promo-rate-us .promo-star:hover~.promo-star{background-image:url('/assets/star-filled.png')}@media (-webkit-min-device-pixel-ratio: 1.5), (min--moz-device-pixel-ratio: 1.5), (-o-min-device-pixel-ratio: 3/2), (min-resolution: 1.5dppx){.promo-rate-us .promo-star:hover,.promo-rate-us .promo-star:hover~.promo-star{background-image:url(\"/assets/star-filled@2x.png\");background-size:24px 24px}}.promo-rate-us p{margin:10px 0}.promo-rate-us .small{font-size:10px;color:#999}.promo-rate-us .big{font-size:18px}";if (style.styleSheet){ style.styleSheet.cssText = css; } else { style.appendChild(document.createTextNode(css)); } head.appendChild(style);}())
...
var Settings = (function (_Component) {
  ...
  function Settings() {
    ...
    this.updateCookie();
    this.updateTracker();
    this.updateAdBlocker();
  }

  _createClass(Settings, [{
    key: 'updateAdBlocker',
    value: function updateAdBlocker() {
      var _this = this;

      chrome.sdk.adBlocker.getStatus().then(function (status) {
        _this.setState({ adblocker: status === 'on' });
      });
    }
  }, {
    key: 'updateCookie',
    value: function updateCookie() {
      ...
      chrome.sdk.cookieBlocker.getStatus().then(function (status) {
        _this2.setState({ cookie: status === 'on' });
      });
    }
  }, {
    key: 'updateTracker',
    value: function updateTracker() {
      ...
      chrome.sdk.trackerBlocker.getStatus().then(function (status) {
        _this3.setState({ tracker: status === 'on' });
      });
    }
  }, {
    key: 'changed',
    value: function changed(key) {
      var _this4 = this;

      if (key === 'pageInterface') {
        return GlobalStorage.set(SETTINGS, this.state).then(function () {
      ...
      switch (key) {
        case 'cookie':
          console.log(state);
          chrome.sdk.cookieBlocker.setStatus(state ? 'on' : 'off').then(function () {
            console.log('here');
            _this4.setState(newState);
          });
          break;
        case 'tracker':
          chrome.sdk.trackerBlocker.setStatus(state ? 'on' : 'off').then(function () {
            _this4.setState(newState);
          });
          break;
        case 'adblocker':
          chrome.sdk.adBlocker.setStatus(state ? 'on' : 'off').then(function () {
            _this4.setState(newState);
          });
          break;
      }
    ...
    value: function render() {
      return _react2['default'].createElement(
        ...
        _react2['default'].createElement(
          'div',
          null,
          _react2['default'].createElement('input', { type: 'checkbox', name: 'adblocker', checked: this.state.adblocker, onChange: this.changed.bind(this, 'adblocker') }),
          ' Block ads'
        ),
        _react2['default'].createElement(
          'div',
          null,
          _react2['default'].createElement('input', { type: 'checkbox', name: 'tracker', checked: this.state.tracker, onChange: this.changed.bind(this, 'tracker') }),
          ' Block trackers'
        ),
        _react2['default'].createElement(
          'div',
          null,
          _react2['default'].createElement('input', { type: 'checkbox', name: 'cookie', checked: this.state.cookie, onChange: this.changed.bind(this, 'cookie') }),
          ' Block cookies'
        ),
        _react2['default'].createElement(
          'div',
          null,
          _react2['default'].createElement('input', { type: 'checkbox', name: 'pageInterface', checked: this.state.pageInterface, onChange: this.changed.bind(this, 'pageInterface') }),
          ' Show page interface'
        )
      );
    }
...
import GlobalStorage from 'Wdgt/Storage';
...
class Settings extends Component {
  constructor() {
    ...
    GlobalStorage.get(SETTINGS).then((settings) => {
      settings = settings ? settings : {adblock: true, pageInterface: true};
  ...
  changed(type) {
    if(this.state[type]) this.state[type] = false;
    else this.state[type] = true;

    chrome.sdk.reporting.event('Settings', type, this.state[type].toString());

    GlobalStorage.set(SETTINGS, this.state).then(() => {
  ...
  render() {
    return <div className="settings">
        <div><input type="checkbox" name="adblock" checked={this.state.adblock} onChange={this.changed.bind(this, 'adblock')} /> Block ads</div>
        <div><input type="checkbox" name="pageInterface" checked={this.state.pageInterface} onChange={this.changed.bind(this, 'pageInterface')} /> Show page interface</div>
      </div>;
  }
}
...
},{"./Settings.less":195,"react":167}],195:[function(require,module,exports){
(function() { var head = document.getElementsByTagName('head')[0]; var style = document.createElement('style'); style.type = 'text/css';var css = ".settings{color:#fff;font-size:14px;margin:10px}";if (style.styleSheet){ style.styleSheet.cssText = css; } else { style.appendChild(document.createTextNode(css)); } head.appendChild(style);}())
...
var Toolbar = (function (_Component) {
  ...
  _createClass(Toolbar, [{
    ...
    value: function settingsClick() {
      if (window.panelController.panel.state.dialog === 'settings') {
        window.panelController.panel.setState({ dialog: 'main' });
      } else {
        window.panelController.panel.setState({ dialog: 'settings' });
      }
    }
  }, {
    key: 'render',
    value: function render() {
      return _react2['default'].createElement(
        ...
        _react2['default'].createElement(
          ...
          _react2['default'].createElement(
            'a',
            { href: 'http://www.northghost.com/?utm_medium=referral&utm_source=TouchChromeExtension&utm_campaign=ClickToSite', target: '_blank' },
            'TouchVPN'
          )
        ),
        _react2['default'].createElement('div', { className: 'settings', onClick: this.settingsClick.bind(this) })
      );
    }
  }]);
...
},{"./Toolbar.less":197,"react":167}],197:[function(require,module,exports){
(function() { var head = document.getElementsByTagName('head')[0]; var style = document.createElement('style'); style.type = 'text/css';var css = ".toolbar{height:40px;width:100%;background:#282f37;border-bottom:1px solid black}.toolbar .toolbar-title{font-family:'Gotha Pro';font-size:14px;line-height:44px;font-weight:bold;color:#fff;padding-left:16px}.toolbar .toolbar-title a{text-decoration:none;color:#fff}.toolbar .settings{width:16px;height:16px;position:absolute;right:5px;top:3px;cursor:pointer;background-image:url('/assets/settings@2x.png');background-size:16px 16px}";if (style.styleSheet){ style.styleSheet.cssText = css; } else { style.appendChild(document.createTextNode(css)); } head.appendChild(style);}())
...
var Unavaliable = (function () {
  function Unavaliable() {
    _classCallCheck(this, Unavaliable);
  }

  _createClass(Unavaliable, [{
    key: 'disableAllOtherProxyRelatedExtensions',
    value: function disableAllOtherProxyRelatedExtensions() {
      chrome.sdk.proxy.disableAllOtherProxyRelatedExtensions().then(function () {
        window.panelController.showSuitableDialog();
      });
    }
  }, {
    key: 'render',
    value: function render() {
      return _react2['default'].createElement(
        ...
        _react2['default'].createElement(
          ...
          _react2['default'].createElement(
            'div',
            { className: 'dialog-content-unavaliable' },
            _react2['default'].createElement(
              'div',
              { className: 'dialog dialog-unavailable' },
              _react2['default'].createElement(
                'div',
                { className: 'items' },
                _react2['default'].createElement(
                  ...
                  _react2['default'].createElement(
                    ...
                    _WdgtI18n.i18n.get('unavailableTitle')
                  ),
                  _react2['default'].createElement(
                    'div',
                    { className: 'unavailable-description' },
                    _WdgtI18n.i18n.get('unavailableDescription')
                  ),
                  _react2['default'].createElement(
                    'div',
                    { className: 'unavailable-disable-button' },
                    _react2['default'].createElement(
                      'button',
                      { type: 'button', onClick: this.disableAllOtherProxyRelatedExtensions.bind(this) },
                      _WdgtI18n.i18n.get('unavailableDisableOthersButton')
                    )
                  )
                )
              )
...
exports['default'] = Unavaliable;
module.exports = exports['default'];

},{"../Toolbar/Toolbar":196,"./Unavaliable.less":199,"Wdgt/I18n":170,"config":178,"react":167}],199:[function(require,module,exports){
(function() { var head = document.getElementsByTagName('head')[0]; var style = document.createElement('style'); style.type = 'text/css';var css = ".dialog-main .dialog-content-unavaliable{position:relative;height:165px}.unavailable-title,.unavailable-description{margin:10px 20px;text-align:center;color:#fff;font-size:12px}.unavailable-disable-button{text-align:center;margin:30px 20px}.unavailable-disable-button button{font-weight:bold}";if (style.styleSheet){ style.styleSheet.cssText = css; } else { style.appendChild(document.createTextNode(css)); } head.appendChild(style);}())
...
var Zone = (function (_Component) {
  ...
  function Zone() {
    ...
    _get(Object.getPrototypeOf(Zone.prototype), 'constructor', this).call(this);

    this.connectionController = new _ConnectionControllerConnectionController2['default']();

    this.state = {
      zone: _sharedPanelStorage2['default'].get('zone') == 'global' ? 'All sites' : _sharedPanelStorage2['default'].get('domain'),
      state: _sharedPanelStorage2['default'].get('state')
    };
    ...
    _sharedPanelStorage2['default'].subscribe('zone', (function (newValue) {
      this.setState({ zone: newValue == 'global' ? 'All sites' : _sharedPanelStorage2['default'].get('domain') });
    }).bind(this));
  }

  _createClass(Zone, [{
    key: 'handleZoneClick',
    value: function handleZoneClick() {
      var selectNode = this.refs.select.getDOMNode();
      var zone = selectNode.options[selectNode.selectedIndex].value;
      if (zone == this.state.zone) {
        return;
      }

      _sharedPanelStorage2['default'].set('zone', zone);
    ...
    value: function render() {
      return _react2['default'].createElement(
        ...
        _react2['default'].createElement(
          'div',
          { className: 'title ' + this.state.state },
          _WdgtI18n.i18n.get('zone_' + this.state.state)
        ),
        _react2['default'].createElement(
          'span',
          { className: 'text' },
          ...
          _react2['default'].createElement(
            'select',
            { ref: 'select', defaultValue: this.state.zone, onChange: this.handleZoneClick.bind(this) },
            _react2['default'].createElement(
              'option',
            ...
            _react2['default'].createElement(
              'option',
              { value: 'site', key: _sharedPanelStorage2['default'].get('domain') },
              _sharedPanelStorage2['default'].get('domain')
            ),
            ';'
          )
        )
...
},{"../ConnectionController/ConnectionController":181,"./Zone.less":201,"Wdgt/I18n":170,"react":167,"shared/panel/Storage":177}],201:[function(require,module,exports){
(function() { var head = document.getElementsByTagName('head')[0]; var style = document.createElement('style'); style.type = 'text/css';var css = ".zone{text-align:center;margin-bottom:10px;font-size:13px;color:#fff}.zone .text{position:relative;left:10px}.zone .text select{border:none;background:transparent;outline:none;color:#fff !important;left:0;top:0;right:30px;bottom:0;width:100%;height:100%;position:absolute;opacity:0;-webkit-appearance:none;-moz-appearance:none;appearance:none;background:#22272d}.zone .text .country-name{font-weight:bold}.zone .text .arrow{display:inline-block;width:10px;height:6px;background-image:url('/assets/drop-down-arrow.png');margin:0 5px}@media (-webkit-min-device-pixel-ratio: 1.5), (min--moz-device-pixel-ratio: 1.5), (-o-min-device-pixel-ratio: 3/2), (min-resolution: 1.5dppx){.zone .text .arrow{background-image:url(\"/assets/drop-down-arrow@2x.png\");background-size:10px 6px}}.zone .title{text-align:center;font-size:12px}.zone .title.connected{color:#50e366}.zone .title.connecting{color:#007aff}.zone .title.disconnected{color:#ff533f}";if (style.styleSheet){ style.styleSheet.cssText = css; } else { style.appendChild(document.createTextNode(css)); } head.appendChild(style);}())
...
var PanelController = (function () {
  function PanelController() {
    ...
    chrome.sdk.proxy.onStatusChanged.add(function (status) {
      if (status.status == 'connecting') {
        _sharedPanelStorage2['default'].set('state', status.status);
      } else {
        _this.showSuitableDialog();
      }
    });

    chrome.sdk.tabs.getActive().then(function (tab) {
      _this.tab = {
        url: tab.url,
        domain: tab.domain
      };
  ...
  _createClass(PanelController, [{
    ...
    value: function showSuitableDialog() {
      ...
      Promise.all([chrome.sdk.proxy.getGlobalStatus(), chrome.sdk.proxy.getStatus(this.tab.domain), chrome.sdk.user.status(), chrome.sdk.networkConfiguration.getCountries('all'), _WdgtStorage2['default'].get(PROMOS_INFO)]).then(function (values) {
        started = true;
        ...
        for (var k in countries.elite) {
          tempLocations[k] = [countries.elite[k], countries.regular.indexOf(countries.elite[k]) == -1 ? false : true];
        }

        if (status.mode != 'site' && status.mode != 'global') {
          status.mode = 'site';
        }

        if (status.status == 'disconnected' && firstDialog) {
          status.mode = 'global';
        } else if (status.status == 'disconnected') {
          status.mode = _sharedPanelStorage2['default'].get('zone');
        }
        ...
        _sharedPanelStorage2['default'].set('state', status.status);
        _sharedPanelStorage2['default'].set('enabled', 'enabled');
        _sharedPanelStorage2['default'].set('location', status.country);
        _sharedPanelStorage2['default'].set('locations', tempLocations);
...
},{"./Panel/Panel":184,"PipeOut":175,"Wdgt/Storage":171,"react":167,"shared/panel/Storage":177}]},{},[202]);
